<script lang="ts">
	...
	import { systemLanguage, AVAILABLE_SYSTEMLANGUAGES } from '@stores/load';
		...
		>
			...
			{#if AVAILABLE_SYSTEMLANGUAGES.length > 5}
				...
				<input
					...
					class="w-1/2 rounded-full border-2 bg-[#242728] uppercase text-white placeholder:text-white focus:ring-2 p-2"
				...
				<datalist id="locales" class="w-1/2 divide-y divide-white uppercase">
					{#each filteredLanguages as locale}
						<option class="uppercase text-red-500">{locale.toUpperCase()}</option>
					{/each}
				</datalist>
			{:else}
				<!-- Dropdown select -->
