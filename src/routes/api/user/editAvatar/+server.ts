...
export const POST: RequestHandler<Partial<Record<string, string>>, string | null> = async ({
	...
	const basePath = '${PUBLIC_MEDIA_FOLDER}/avatar';
	...
	const newPath = `${basePath}/${UserSchema._id}_${new Date().getTime()}_avatar.webp`;

	// Get the current avatar path from the user object
	const currentAvatar = UserSchema.avatar;
	...
	const trashPath = 'trash/{PUBLIC_MEDIA_FOLDER}/avatar';
