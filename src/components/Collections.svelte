...
{#each filtered as item, index}
	...
	<div use:setHeight class="overflow-hidden " class:expand={expanded[index]}>
		...
		{#each item.collections as _collection}
			<p
				class="relative cursor-pointer border-b border-surface-200 bg-white p-0 text-center text-black last:mb-1 last:border-b-0 hover:bg-[#65dfff] hover:text-white dark:hover:bg-[#65dfff] dark:hover:text-white dark:bg-surface-400"
				on:click={async () => {
