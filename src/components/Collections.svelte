...
<div on:click={(e)=>{expanded[index] = !expanded[index];expanded[index]?setOverflowY(e,true):setOverflowY(e,false)}} class="relative p-3 px-10 bg-[#ff7254] cursor-pointer text-center max-w-[200px]  arrow" class:arrow_up={expanded[index]}>
    {item.category}
</div>
    <div use:setHeight class="overflow-hidden"  class:expand={expanded[index]}>
        {#each item.collections as _collection}
            <p class="text-black cursor-pointer py-2 text-center bg-white hover:bg-[#65dfff] hover:text-white" on:click={()=>{fields = get_fields(_collection);collection =_collection;showFields=false}}>{_collection.name}</p>
        {/each}
    </div>
{/each}
